...
async function getPost(req, res) {
	...
	pg.query(query, [req.params.image_id, req.token.id])
		.then(db => {

			if (db) res.status(200).json(db.rows[0]);
			else res.sendStatus(400);
		}).catch(err => {
			res.sendStatus(500);
			console.log(err);
...
async function editImage(req, res) {
	...
	pg.query(query, args)
		.then(db => db.rowCount ? res.sendStatus(201) : res.sendStatus(404))
...
async function deleteImage(req, res) {
	...
	pg.query(query, [req.token.id, req.params.image_id])
		.then(db => db.rowCount ? res.sendStatus(200) : res.sendStatus(400))
		.then(() => { require('../workers/elastic-search.worker').updateImagesIndeces() })
